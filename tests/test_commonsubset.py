...
from honeybadgermpc.protocols.commonsubset import commonsubset
...
async def make_commonsubset(sid, pid, n, f, pk, sk, input_msg, send, recv, bcast):
    ...
    tasks.append(asyncio.create_task(_recv()))
    ...
    task_lists = await asyncio.gather(*[_setup(j) for j in range(n)])
    for task_list in task_lists:
        tasks.extend(task_list)

    return commonsubset(pid, n, f, rbc_outputs,
                        [_.put_nowait for _ in aba_inputs],
                        [_.get for _ in aba_outputs]), tasks
