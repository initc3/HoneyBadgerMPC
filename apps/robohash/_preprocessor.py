...
class PreProcessor:
    ...
    def __init__(
        ...
        self.w3 = w3
        self._preprocessing = _create_task(self._offline_cryptodna_loop())
        self.db = db
    ...
    async def start(self):
        ...
        # await self._offline_cryptodna_loop()
    ...
    # FIXME rename to: async def _private_genomes_loop(self):
    async def _offline_cryptodna_loop(self):
        ...
        while True:
            ...
            while True:
                cryptodna_available = contract_concise.cryptodna_available()
                totalmasks = contract_concise.preprocess()

                logging.info(f"available crypto DNAs: {cryptodna_available}")
                logging.info(f"total crypto DNAs: {totalmasks}")
                # Policy: try to maintain a buffer of 10 crypto DNAs
                target = 10 * K + 2
                if cryptodna_available < target:
                    break
                # already have enough crypto DNAs, sleep
                await asyncio.sleep(5)

            # Step 1. II) Run Randousha
            logging.info(
                f"[{self.myid}] totalmasks: {totalmasks} \
                cryptodna available: {cryptodna_available} \
                target: {target} Initiating Randousha {k * (n - 2*t)}"
            )
            send, recv = self.get_send_recv(f"preproc:cryptodna:{preproc_round}")
            ...
            self.elements["cryptodna"] += rs_t
            self.db[b"cryptodna"] = pickle.dumps(self.elements["cryptodna"])
